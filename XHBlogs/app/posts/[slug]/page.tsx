...
// 读取文章 frontmatter（轻量，仅供 SEO 元数据使用）
function getPostMeta(slug: string) {
  ...
  const cover = data.cover
    ? data.cover.startsWith('http')
      ? data.cover
      : `${SITE_URL}${data.cover}`
    : `${SITE_URL}/cover-default.svg`;
  return {
    title: data.title || '文章',
    description: (data.description || data.excerpt || data.title || '').toString(),
    date: data.date,
...
export async function generateMetadata({
  ...
  return {
    ...
    keywords: meta.tags,
    authors: [{ name: 'Bortala' }],
    alternates: { canonical: url },
    openGraph: {
      type: 'article',
      locale: 'zh_CN',
      url,
      siteName: 'Bortala の 宝藏之地',
      title: meta.title,
      description: meta.description,
...
export default async function Post({ params }: { params: Promise<{ slug: string }> }) {
  ...
  const articleJsonLd = {
    ...
    description: postData.contentHtml.replace(/<[^>]+>/g, '').slice(0, 200) || siteConfig.bio,
    ...
    author: { '@type': 'Person', name: siteConfig.authorName, url: 'https://blog.955827.xyz' },
    publisher: { '@type': 'Person', name: siteConfig.authorName, url: 'https://blog.955827.xyz' },
