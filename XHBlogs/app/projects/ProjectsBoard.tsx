        ...
        <AnimatePresence>
          {filteredProjects.map((project) => (
            <motion.div
              ...
              className="h-full"
            >
              <div className="relative h-full rounded-3xl bg-white/60 dark:bg-slate-800/50 backdrop-blur-xl border border-white/40 dark:border-white/10 shadow-xl overflow-hidden hover:shadow-indigo-500/20 transition-all duration-700 hover:-translate-y-1 group p-6 md:p-8">
                {/* 装饰性光晕 */}
                <div className="absolute -top-10 -right-10 w-32 h-32 bg-indigo-500/10 rounded-full blur-2xl group-hover:bg-indigo-500/20 transition-colors duration-700 pointer-events-none"></div>

                {/* 整卡拉伸链接：优先跳线上站，无站则回退 GitHub 仓库 */}
                <a
                ...
                {/* 内容（位于拉伸链接之下，链接透明故可见；group-hover 仍生效） */}
                <div className="relative z-0">
                  ...
                  <p className="text-sm text-slate-700 dark:text-slate-300 font-serif leading-relaxed line-clamp-3 mb-6 min-h-[60px]">
                    {project.description}
                  </p>
                ...
                {/* GitHub 角标：独立链接，位于拉伸链接之上，保留代码入口 */}
                <a
                  href={project.githubUrl}
                  target="_blank"
                  rel="noopener noreferrer"
                  aria-label={`${project.name} 的 GitHub 仓库`}
                  className="absolute top-6 right-6 md:top-8 md:right-8 z-20 w-8 h-8 flex items-center justify-center text-slate-400 group-hover:text-slate-800 dark:group-hover:text-white transition-colors"
                >
